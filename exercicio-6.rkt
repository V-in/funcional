...
;; (filter p lst) retorna uma lista contendo os itens de lst que   (time (empty? '()))
...
(define (filter p lst)
  (define (recur p lst acc)
    (cond [(empty? lst) acc]
          [(p (first lst)) (recur p (rest lst) (cons (first lst) acc))]
          [else (recur p (rest lst) acc)]))
  (reverse (recur p lst '())))
...
(define (filtra-par lst) (filter even? lst))

(define (filtra-impar lst) (filter odd? lst))
...
(define (filtra-comp p lst) (filter (lambda (x) (not (p x))) lst))
...
(define (tamanho-lista-strings lstr)
  (foldl (lambda (x y) (+ y (string-length x))) 0 lstr))
...
(define (bigStr n)
  (define (generate acc n)
    (cond [(= 0 n) acc]
          [else (generate (cons "Uma string qualquer" acc) (- n 1))]))
  (generate '() n))

(define (tamanho-lista-strings-2 lstr) (string-length (apply string-append lstr)))

(time (tamanho-lista-strings (bigStr 1000000)))
(time (tamanho-lista-strings-2 (bigStr 1000000)))
...
(define (todos? p lst)
  (foldl (lambda (x y) (and (p x) y)) #t lst))
...
(define (partition p lst)
  (define (recur p lst acc1 acc2)
    (cond [(empty? lst) (list (reverse acc1) ( reverse acc2 )) ]
          [(p (first lst)) (recur p (rest lst) (cons (first lst) acc1) acc2)]
          [else (recur p (rest lst) acc1 (cons (first lst) acc2))]))
  (recur p lst '() '()))
...
(define (menor-quociente x lst)
  (define (recur x lst menor)
    (cond [(empty? lst) menor]
          [(= 0 (first lst)) "Divisao por 0"]
          [(< (/ x (first lst)) menor) (recur x (rest lst) (/ x (first lst)))]
          [else (recur x (rest lst) menor)]))
  (if (empty? lst)
      '()
      (recur x (rest lst) (/ x (first lst)))))
 
(define-test-suite test-menor-quociente
  (test-equal? "Lista vazia"
               (menor-quociente 12 '())
               '())
  (test-equal? "Numeros inteiros"
               (menor-quociente 12 '(2 3 4))
               3)
  (test-equal? "Positivos e negativos"
               (menor-quociente 1 '(100 -30 40 32 -2))
               -1/2)
  (test-equal? "Menores que 1"
               (menor-quociente 2 '(-100 -30 -40 -32 -2))
               -1)
  (test-equal? "Zero"
               (menor-quociente 1 '(100 -30 40 32 0))
               "Divisao por 0"))
